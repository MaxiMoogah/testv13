        ...
        if gift_card_id.card_value >= amount:
            if not gift_card_session_id:
                ...
                if sale_order.amount_total >= amount:
                    ...
                    value = {'success': True}
                    if gift_card_id.expiry_date and gift_card_id.expiry_date <= date.today():
                        value.update({'expiry_error': 'Your Gift Card has been Expired!'})
                    return simplejson.dumps(value)
                else:
                ...
                if sale_order.amount_total >= amount:
                    ...
                    value = {'success': True}
                    if gift_card_id.expiry_date and gift_card_id.expiry_date <= date.today():
                        value.update({'expiry_error': True})
                    return simplejson.dumps(value)
                else:
                    return simplejson.dumps({'error': True})
        else:
