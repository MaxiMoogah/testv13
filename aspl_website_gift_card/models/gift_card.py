...
class WebsiteGiftCard(models.Model):
    ...
    default_expire_month = fields.Char(string="Default Expire Month", default_model='website.gift.card')
    ...
    def create(self, vals):
        ...
        template_id = self.env['ir.model.data'].get_object_reference('aspl_website_gift_card',
...
class SaleOrder(models.Model):
    ...
    def write(self, vals):
        number_of_months = int(self.env['ir.config_parameter'].sudo().get_param('aspl_website_gift_card.expire_default_month'))

        if number_of_months:
            expiry_date = date.today() + relativedelta(months=number_of_months)
        else:
            expiry_date = False
        for so in self:
...
class ResConfigSettings(models.TransientModel):
    ...
    expire_default_month = fields.Integer(string="Default Expire Month", default_model='')
    ...
    def get_values(self):
        ...
        res.update(
            {'expire_default_month': int(param_obj.sudo().get_param('aspl_website_gift_card.expire_default_month') or False)})
        return res

    @api.model
    def set_values(self):
        ICPSudo = self.env['ir.config_parameter'].sudo()
        ICPSudo.set_param("aspl_website_gift_card.expire_default_month", self.expire_default_month)
