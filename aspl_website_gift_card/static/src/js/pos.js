odoo.define('aspl_website_gift_card.pos', function (require) {
"use strict";
    ...
    var round_pr = utils.round_precision;
      ...
      models.PosModel.prototype.models.push(
        {
        model:  'website.gift.card',
        fields: ['card_no','card_value','customer_id','issue_date','expiry_date','is_active','pin_no'],
    ...
    var GiftCardListScreenWidget = screens.ScreenWidget.extend({
        ...
        /*click_edit_giftcard: function(event){
            ...
            var result = self.pos.db.get_card_by_id(card_id);
            if (result) {
                self.gui.show_popup('edit_card_popup',{'card_id':card_id,'card_no':result.card_no,'expiry_date':result.expiry_date});
            }
        },*/
        ...
        render_list: function(gift_cards){
            ...
            if(self.date !== "" && self.date !== "all"){
                ...
                for (var i=0; i<gift_cards.length;i++){
                    var date_expiry = gift_cards[i].expiry_date;
            ...
            if(self.expiry_date !== "" && self.expiry_date !== "all"){
                var y = [];
                for (var i=0; i<gift_cards.length;i++){
                    var date_expiry = gift_cards[i].expiry_date;
                    var date_issue = gift_cards[i].issue_date;
                    if(self.expiry_date == date_expiry){
                        y.push(gift_cards[i]);
                    }
                }
                gift_cards = y;
    ...
    var CreateCardPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            ...
            if(self.pos.config.default_exp_date && !self.panding_card){
                ...
                self.$('#text_expiry_date').val(new_date);
            }
        ...
        click_confirm: function(){
            ...
            if(!card_number){
                ...
                alert("Enter gift card pin");
                return;
            }else if(card_number.length < 10){
                alert("Enter 10 digit card number")
            }else{
                var params = {
                        model: 'website.gift.card',
                        method: 'search_read',
                ...
                rpc.query(params, {async: false}).then(function(gift_count){
                    ...
                    if(gift_count > 0){
                        ...
                        rpc.query(param_config, {async: false}).then(function(result){
                            if(result && result.expire_default_month){
                                var x = result.expire_default_month; //or whatever offset
                                var CurrentDate = new Date();
                                CurrentDate.setMonth(CurrentDate.getMonth() + x);
                                expiry_date =  moment(CurrentDate, 'YYYY/MM/DD').format('YYYY-MM-DD');
                            }else{
                                expiry_date = false;
                            }
                            if(checkbox_paid.checked){
                                $('#text_amount').focus();
                                var input_amount =self.$('#text_amount').val();
                                if(input_amount){
           ...
           $('#text_gift_card_no').keypress(function(e) {
               if (e.which == 13 && $(this).val()) {
                    ...
                    var params = {
                        model: 'website.gift.card',
                        method: 'search_read',
                        domain: [['card_no', '=', code], ['issue_date', '<=', today]],
                    }
                    rpc.query(params, {async: false})
//                    new Model('aspl.gift.card').get_func('search_read')([['card_no', '=', code], ['expire_date', '>=', today]])
                    .then(function(res){
                        if(res.length > 0){
                            if (res[0] && res[0].expiry_date){
                                if(res[0].expiry_date >= today){
    ...
    screens.ReceiptScreenWidget.include({
        show: function(){
            ...
            if( barcode_free_val){
                ...
                $("." + barcode).barcode(barcode.toString(), "code128");
